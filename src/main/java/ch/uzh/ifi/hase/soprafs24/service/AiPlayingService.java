...
public class AiPlayingService {
    ...
    public GameStats getCardStatsByCode(String cardCode, Game game, GameStatsRepository repo) {
        if (cardCode == null || cardCode.length() != 2) {
            throw new IllegalArgumentException("Invalid card code: " + cardCode);
        }

        String rankStr = cardCode.substring(0, 1);
        String suitStr = cardCode.substring(1, 2);

        Rank rank = Rank.valueOf(rankStr);
        Suit suit = Suit.valueOf(suitStr);

        return repo.findByRankAndSuitAndGame(rank, suit, game);
    }

    public String pickSafestWinningCard(String[] legalCards, Game game, MatchPlayer matchPlayer,
            GameStatsRepository repo) {
        int mySlot = matchPlayer.getMatchPlayerSlot();
        List<String> currentTrick = game.getCurrentTrick();
        String leadSuit = game.getSuitOfFirstCardInCurrentTrick(); // "H", "S", etc.
        List<Integer> trickOrder = game.getTrickMatchPlayerSlotOrder();
        int myIndex = trickOrder.indexOf(mySlot);
        List<Integer> playersAfterMe = trickOrder.subList(myIndex + 1, trickOrder.size());

        String safestCard = null;

        for (String cardCode : legalCards) {
            // Must follow lead suit if not leading
            if (!currentTrick.isEmpty() && !cardCode.endsWith(leadSuit)) {
                continue;
            }

            // Check if this card is currently winning
            boolean wouldCurrentlyWin = true;
            for (String playedCard : currentTrick) {
                if (!playedCard.endsWith(leadSuit))
                    continue;

                if (CardUtils.compareCards(playedCard, cardCode) > 0) {
                    wouldCurrentlyWin = false;
                    break;
                }
            }
            ...
            // Safe so far — now check if anyone after me might beat it
            BitSet holders = getPossibleHolders(cardCode, game, mySlot, repo);
            boolean canBeBeaten = false;
            for (int slot : playersAfterMe) {
                if (holders.get(slot - 1)) {
                    canBeBeaten = true;
                    break;
                }
            }
            ...
            if (!canBeBeaten) {
                return cardCode; // Safe to play this card and likely win
            }

            // Fallback if no safe win is found
            if (safestCard == null) {
                safestCard = cardCode;
            }
        }

        return safestCard != null ? safestCard : legalCards[0];
    }
